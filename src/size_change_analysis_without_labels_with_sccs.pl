...
assert_idempotent_graphs :-
   retractall(deleted(_,_,_,_)), 
   vprintln('Removing Superfluous Idempotent Graphs: '), reset_count,
   existing_sc_graph(_Hash1,P,N,P,N,Arrows1),
   \+(deleted(P,N,Arrows1)),
   %SC1 = sc(from(P,N),to(P,N),arrows(Arrows1)),
   %sc_concat(SC1,SC1,SC1), /* it is idempotent */
   existing_sc_graph(Hash2,P,N,P,N,Arrows2), Arrows2 \= Arrows1, 
   %SC2 = sc(from(P,N),to(P,N),arrows(Arrows2)),
   %sc_concat(SC2,SC2,SC2), /* it is idempotent */
   %nl,nl,print(SC1),nl,print(SC2),nl,nl,
   %print(weaker_arrows(Arrows1,Arrows2)),nl,nl,
   (weaker_arrows(Arrows1,Arrows2) -> true ; fail),
   %print(removing(P,N,Arrows2)),nl,
   retract(existing_sc_graph(Hash2,P,N,P,N,Arrows2)),
   assert(deleted(P,N,Arrows2)),
   inc_count,
   fail.
/* assert_idempotent_graphs :- 
  findall((P,N,Arrows),existing_sc_graph(_Hash,P,N,P,N,Arrows),L),
  print(L),halt.
*/
assert_idempotent_graphs :-
...
idempotent(sc(from(P,N),to(P,N),arrows(Arrows))) :-
   sc_concat(sc(from(P,N),to(P,N),arrows(Arrows)),sc(from(P,N),to(P,N),arrows(Arrows)),sc(from(P,N),to(P,N),arrows(Arrows2))),
   same_edges(Arrows,Arrows2).

same_edges([],[]).
same_edges([(N,M,_Label1)|R],[(N,M,_Label2)|R2]) :- same_edges(R,R2).
